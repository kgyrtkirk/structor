...
$BEELINE -f 14queens_create_int.sql
...
for type in tinyint smallint int bigint float double decimal; do
	...
	$BEELINE -f 14queens_create_$type.sql
	echo "Run $type vectorization on"
	$BEELINE -f 14queens_run.sql --hivevar VECTORIZATION_ON=true
	echo "Run $type vectorization off"
	$BEELINE -f 14queens_run.sql --hivevar VECTORIZATION_ON=false
done
